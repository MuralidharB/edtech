...
from app.crud import lesson as crud
...
def get_lessons_for_course(course_id: UUID, db: Session = Depends(get_db)):
    return lesson_crud.get_all_by_course(db, course_id)

@router.get("/{lesson_id}", response_model=LessonOut)
def get_lesson_detail(lesson_id: UUID, db: Session = Depends(get_db)):
...
def get_next_lesson(lesson_id: UUID, db: Session = Depends(get_db)):
    return lesson_crud.get_next(db, lesson_id)

@router.get("/{lesson_id}/recommendation", response_model=LessonOut)
def recommend_next_lesson(lesson_id: UUID, student_id: UUID, db: Session = Depends(get_db)):
